# CocktailRecipe SDK Search entity
...
class SearchEntity
  ...
  def make
    opts = @_entopts.dup
    SearchEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CocktailRecipeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Search, Hash] the current Search data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CocktailRecipeHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Search fields)
  def match_get
  ...
  # List Search items matching the given filter.
  #
  # @param reqmatch [SearchListMatch, Hash, nil] match filter (any subset of
  #   Search fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Search>, Array] the matching Search items; raises CocktailRecipeError on failure
