...
class RandomDirectTest < Minitest::Test
  def test_direct_list_random
    setup = random_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-random", setup[:live] ? "live" : "unit")
    if _should_skip
...
def random_direct_setup(mockres)
