...
class FilterDirectTest < Minitest::Test
  def test_direct_list_filter
    setup = filter_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-filter", setup[:live] ? "live" : "unit")
    if _should_skip
...
def filter_direct_setup(mockres)
