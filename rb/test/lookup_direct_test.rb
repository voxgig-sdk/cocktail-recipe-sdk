...
class LookupDirectTest < Minitest::Test
  def test_direct_list_lookup
    setup = lookup_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-lookup", setup[:live] ? "live" : "unit")
    if _should_skip
...
def lookup_direct_setup(mockres)
