# CocktailRecipe SDK utility: make_request
...
module CocktailRecipeUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CocktailRecipeResponse.new({})
    ...
    if fetch_err
      ...
      response = CocktailRecipeResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CocktailRecipeResponse.new(fetched)
    else
