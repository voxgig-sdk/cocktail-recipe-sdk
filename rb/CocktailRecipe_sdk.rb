...
require_relative 'CocktailRecipe_types'
...
class CocktailRecipeSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = CocktailRecipeHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = CocktailRecipeHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, CocktailRecipeFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    CocktailRecipeUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = CocktailRecipeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = CocktailRecipeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = CocktailRecipeHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => CocktailRecipeError.new(
        "#{op}_allow",
        "CocktailRecipeSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    rescue CocktailRecipeError => err
    ...
    if fetched.is_a?(Hash)
      status = CocktailRecipeHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = CocktailRecipeError.new(
        "graphql_error", "CocktailRecipeSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = CocktailRecipeSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
