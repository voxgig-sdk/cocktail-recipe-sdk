...
The Lua SDK for the CocktailRecipe API. Provides an entity-oriented interface using Lua conventions.
...
```lua
local result, err = client:Filter(nil):list(nil, nil)
...
local result, err = client:CocktailRecipe(nil):load(
  { id = "test01" }, nil
)
