...
describe("FilterEntity", function()
  ...
  it("should run basic flow", function()
    ...
    if setup.synthetic_only then
      pending("live entity test uses synthetic IDs from fixture — set COCKTAIL_RECIPE_TEST_FILTER_ENTID JSON to run live")
      return
    end
    local client = setup.client

    -- Bootstrap entity data from existing test data.
    local filter_ref01_data_raw = vs.items(helpers.to_map(
      vs.getpath(setup.data, "existing.filter")))
    local filter_ref01_data = nil
    if #filter_ref01_data_raw > 0 then
      filter_ref01_data = helpers.to_map(filter_ref01_data_raw[1][2])
    end
...
function filter_basic_setup(extra)
  ...
  local entid_env_raw = os.getenv("COCKTAIL_RECIPE_TEST_FILTER_ENTID")
