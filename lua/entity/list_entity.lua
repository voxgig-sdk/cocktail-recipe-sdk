...
local ListEntity = {}
ListEntity.__index = ListEntity


function ListEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, ListEntity)
  self._name = "list"
  self._client = client
...
function ListEntity:get_name()
  return self._name
end


function ListEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return ListEntity.new(self._client, opts)
end
...
function ListEntity:mark_deleted()
  self._deleted = true
end


function ListEntity:deleted()
  return true == self._deleted
end


function ListEntity:data_set(args)
  if args ~= nil then
...
function ListEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function ListEntity:match_set(args)
  if args ~= nil then
...
function ListEntity:stream(action, args, callopts)
...
---@param reqmatch ListListMatch
---@param ctrl? table
---@return List[]
---@return string? err
function ListEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function ListEntity:_run_op(ctx, post_done)
  local utility = self._utility
