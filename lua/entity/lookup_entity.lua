...
local LookupEntity = {}
LookupEntity.__index = LookupEntity


function LookupEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, LookupEntity)
  self._name = "lookup"
  self._client = client
...
function LookupEntity:get_name()
  return self._name
end


function LookupEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return LookupEntity.new(self._client, opts)
end
...
function LookupEntity:mark_deleted()
  self._deleted = true
end


function LookupEntity:deleted()
  return true == self._deleted
end


function LookupEntity:data_set(args)
  if args ~= nil then
...
function LookupEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function LookupEntity:match_set(args)
  if args ~= nil then
...
function LookupEntity:match_get()
...
function LookupEntity:stream(action, args, callopts)
...
function LookupEntity:_run_op(ctx, post_done)
  local utility = self._utility
