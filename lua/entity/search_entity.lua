...
local SearchEntity = {}
SearchEntity.__index = SearchEntity


function SearchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, SearchEntity)
  self._name = "search"
  self._client = client
...
function SearchEntity:get_name()
  return self._name
end


function SearchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return SearchEntity.new(self._client, opts)
end
...
function SearchEntity:mark_deleted()
  self._deleted = true
end


function SearchEntity:deleted()
  return true == self._deleted
end


function SearchEntity:data_set(args)
  if args ~= nil then
...
function SearchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function SearchEntity:match_set(args)
  if args ~= nil then
...
function SearchEntity:match_get()
...
function SearchEntity:stream(action, args, callopts)
...
function SearchEntity:_run_op(ctx, post_done)
  local utility = self._utility
