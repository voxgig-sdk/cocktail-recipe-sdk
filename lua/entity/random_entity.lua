-- CocktailRecipe SDK Random entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local RandomEntity = {}
RandomEntity.__index = RandomEntity


function RandomEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, RandomEntity)
  self._name = "random"
  self._client = client
...
function RandomEntity:get_name()
  return self._name
end


function RandomEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return RandomEntity.new(self._client, opts)
end
...
function RandomEntity:mark_deleted()
  self._deleted = true
end


function RandomEntity:deleted()
  return true == self._deleted
end


function RandomEntity:data_set(args)
  if args ~= nil then
...
function RandomEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function RandomEntity:match_set(args)
  if args ~= nil then
...
function RandomEntity:match_get()
...
function RandomEntity:stream(action, args, callopts)
...
function RandomEntity:_run_op(ctx, post_done)
  local utility = self._utility
