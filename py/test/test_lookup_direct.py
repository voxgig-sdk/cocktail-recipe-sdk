...
class TestLookupDirect:

    def test_should_direct_list_lookup(self):
        setup = _lookup_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-lookup", "live" if setup["live"] else "unit")
        if _skip:
...
def _lookup_direct_setup(mockres):
