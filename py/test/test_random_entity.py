...
class TestRandomEntity:
    ...
    def test_should_stream(self):
        ...
        seen = list(base.Random(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.Random(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set COCKTAIL_RECIPE_TEST_RANDOM_ENTID JSON to run live")
        client = setup["client"]
...
def _random_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("COCKTAIL_RECIPE_TEST_RANDOM_ENTID"))
