# CocktailRecipe SDK test runner
...
class CocktailRecipeTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CocktailRecipeTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CocktailRecipeTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CocktailRecipeTestRunner.getenv("COCKTAIL_RECIPE_TEST_LIVE")
        override = CocktailRecipeTestRunner.getenv("COCKTAIL_RECIPE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CocktailRecipeTestRunner.getenv(key)
        ...
        explain = CocktailRecipeTestRunner.getenv("COCKTAIL_RECIPE_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if CocktailRecipeTestRunner._test_control is not None:
            return CocktailRecipeTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CocktailRecipeTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CocktailRecipeTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CocktailRecipeTestRunner.load_test_control()
...
def load_env_local():
    CocktailRecipeTestRunner.load_env_local()


def env_override(m):
    return CocktailRecipeTestRunner.env_override(m)


def entity_data(v):
    return CocktailRecipeTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CocktailRecipeTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CocktailRecipeTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CocktailRecipeTestRunner.load_test_control()


def live_client_options():
    return CocktailRecipeTestRunner.live_client_options()


def live_delay_ms():
    return CocktailRecipeTestRunner.live_delay_ms()
