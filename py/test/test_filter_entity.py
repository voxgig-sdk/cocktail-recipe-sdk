...
class TestFilterEntity:
    ...
    def test_should_stream(self):
        ...
        base = CocktailRecipeSDK.test(seed, None)
        seen = list(base.Filter(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            sdk = CocktailRecipeSDK.test(
                seed, {"feature": {"streaming": {"active": True}}})
            got = []
            for item in sdk.Filter(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set COCKTAIL_RECIPE_TEST_FILTER_ENTID JSON to run live")
        client = setup["client"]
...
def _filter_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("COCKTAIL_RECIPE_TEST_FILTER_ENTID"))
    ...
    _live = env.get("COCKTAIL_RECIPE_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("COCKTAIL_RECIPE_TEST_EXPLAIN") == "TRUE",
        "live": _live,
