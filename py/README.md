...
The Python SDK for the CocktailRecipe API. Provides an entity-oriented interface following Pythonic conventions.
...
```python
result, err = client.Filter(None).list(None, None)
if err:
    raise Exception(err)
...
result, err = client.CocktailRecipe(None).load(
    {"id": "test01"}, None
)
