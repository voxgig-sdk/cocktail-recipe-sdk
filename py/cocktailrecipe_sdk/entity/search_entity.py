...
from cocktailrecipe_sdk.cocktailrecipe_types import (
    Search,
    SearchListMatch,
)
...
class SearchEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return SearchEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Search]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Search().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
