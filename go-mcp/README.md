...
[MCP](https://modelcontextprotocol.io) server exposing the CocktailRecipe SDK as
two agent tools — `cocktail-recipe_list` and `cocktail-recipe_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cocktail-recipe-mcp)
make build

# 2. Provide credentials via the environment
export COCKTAIL_RECIPE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user cocktail-recipe \
  -- /absolute/path/to/cocktail-recipe-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./cocktail-recipe-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// cocktail-recipe_list: first page of records
{ "entity": "filter" }
{ "entity": "filter", "query": { } }

// cocktail-recipe_load: one record by id
{ "entity": "filter", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cocktail-recipe-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user cocktail-recipe \
     -- "$PWD"/dist/*/cocktail-recipe-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `cocktail-recipe_list` and `cocktail-recipe_load` tools now appear
   in new sessions. Ask the agent to *"list filter using cocktail-recipe"*
   and it calls `cocktail-recipe_list` with `{"entity":"filter"}`.
...
```sh
export COCKTAIL_RECIPE_APIKEY=sk_live_xxx            # API key
export COCKTAIL_RECIPE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `cocktail-recipe_list` tool
...
### Call the `cocktail-recipe_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `cocktail-recipe_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `cocktail-recipe_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `COCKTAIL_RECIPE_APIKEY` | API key sent with every request. |
| `COCKTAIL_RECIPE_BASE` | Optional override of the API base URL. |

### Entities

The 5 entities valid as the `entity` argument:

filter | list | lookup | random | search

### Smoke test via HTTP (raw JSON-RPC)

```sh
./cocktail-recipe-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"cocktail-recipe_load","arguments":{"entity":"filter","query":{"id":1}}}}'
```
