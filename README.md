...
Provides access to a free cocktail recipe API with various endpoints for searching cocktails, ingredients, and more.

Learn more about Cocktail Recipe API at [thecocktaildb.com](https://thecocktaildb.com).

This is an unofficial SDK for the Cocktail Recipe public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — Filter, List, Lookup, Random and Search — that you
...
const items = await client.Filter().list()
```
...
// filters is an array of Filter entities, populated with mock data
// — call filters[0].data() for the record itself
console.log(filters)
```
...
```go
client := sdk.Test()
result, err := client.Filter(nil).List(
    nil, nil,
)
```
...
| Ruby | `voxgig-sdk-cocktail-recipe` | publish pending — [install from git tag](https://github.com/voxgig-sdk/cocktail-recipe-sdk/releases) |
| Lua | `voxgig-sdk-cocktail-recipe` | publish pending — [install from git tag](https://github.com/voxgig-sdk/cocktail-recipe-sdk/releases) |
| Go CLI | `github.com/voxgig-sdk/cocktail-recipe-sdk/go-cli` | `go install github.com/voxgig-sdk/cocktail-recipe-sdk/go-cli/cmd/cocktail-recipe@latest` |
| Go MCP server | `github.com/voxgig-sdk/cocktail-recipe-sdk/go-mcp` | `go get github.com/voxgig-sdk/cocktail-recipe-sdk/go-mcp@latest` |
...
// List all filters (returns FilterEntity[] — .data() for the record)
const filters = await client.Filter().list()
...
$filters = $client->Filter()->list();
print_r(array_map(fn($item) => $item->data_get(), $filters));
```
...
- Upstream API: [https://www.thecocktaildb.com/api/json/v1/1](https://www.thecocktaildb.com/api/json/v1/1)
