...
boru-driven command-line client **and** interactive REPL for the CocktailRecipe
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/cocktail-recipe-cli)
make build

# 2. See usage (words, entities, env vars)
./cocktail-recipe-cli --help

# 3. Provide credentials once, via the environment
export COCKTAIL_RECIPE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./cocktail-recipe-cli list filter
./cocktail-recipe-cli list list

# 5. Override the API base URL for a single call
COCKTAIL_RECIPE_BASE=https://api.example.com ./cocktail-recipe-cli list filter

# 6. No arguments -> interactive REPL
./cocktail-recipe-cli
cocktail-recipe> list filter
cocktail-recipe> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/cocktail-recipe-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/cocktail-recipe-cli list filter
   ```
...
Run with no arguments to open a REPL (prompt `cocktail-recipe>`). Each line is
evaluated as its own boru expression:

```text
$ ./cocktail-recipe-cli
cocktail-recipe> list filter
cocktail-recipe> /help
cocktail-recipe> /quit
```
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `filter`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/cocktail-recipe-cli`. |
...
(`./cocktail-recipe-cli <expr>`) and interactively (the REPL), and expressions compose the
