// Command CocktailRecipe-cli is a CLI + REPL for the CocktailRecipe SDK,
...
const prompt = "cocktail-recipe"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "filter list lookup random search"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      COCKTAIL_RECIPE_APIKEY (api key), COCKTAIL_RECIPE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: COCKTAIL_RECIPE_APIKEY carries the API key and
	// COCKTAIL_RECIPE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("COCKTAIL_RECIPE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("COCKTAIL_RECIPE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCocktailRecipeSDK(opts)
