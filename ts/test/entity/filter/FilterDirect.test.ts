...
describe('FilterDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new CocktailRecipeSDK({
  ...
  test('direct-list-filter', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-filter', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['COCKTAIL_RECIPE_TEST_FILTER_ENTID']
