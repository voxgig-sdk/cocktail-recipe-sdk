...
//     (\`new CocktailRecipeSDK(...)\` / \`CocktailRecipeSDK.test(...)\` become
//     \`CocktailRecipeSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'CocktailRecipeSDK'
...
const TEST_SEED = {"entity":{"filter":{"test01":{"id":"test01"}},"list":{"test01":{"id":"test01"}},"lookup":{"test01":{"id":"test01"}},"random":{"test01":{"id":"test01"}},"search":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new CocktailRecipeSDK(...)\` and any
// \`CocktailRecipeSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+CocktailRecipeSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/CocktailRecipeSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
