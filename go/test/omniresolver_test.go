...
import (
	...
	omni "github.com/voxgig-sdk/cocktail-recipe-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CocktailRecipeError); is {
				out := map[string]any{
					"name":    "CocktailRecipeError",
...
func omniCtx(arg any, client *sdk.CocktailRecipeSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CocktailRecipeSDK); is {
			client = live
