...
func TestListEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.List(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.List(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		listRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.list")))
...
func listBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COCKTAIL_RECIPE_TEST_LIST_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COCKTAIL_RECIPE_TEST_LIST_ENTID"])
