...
func TestRandomEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		randomRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.random")))
...
func randomBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COCKTAIL_RECIPE_TEST_RANDOM_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COCKTAIL_RECIPE_TEST_RANDOM_ENTID"])
