...
func TestLookupEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Lookup(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Lookup(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set COCKTAIL_RECIPE_TEST_LOOKUP_ENTID JSON to run live")
			return
		}
		client := setup.client

		// Bootstrap entity data from existing test data (no create step in flow).
		lookupRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath("existing.lookup", setup.data)))
		var lookupRef01Data map[string]any
		if len(lookupRef01DataRaw) > 0 {
			lookupRef01Data = core.ToMapAny(lookupRef01DataRaw[0][1])
		}
...
func lookupBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("COCKTAIL_RECIPE_TEST_LOOKUP_ENTID")
	...
	idmapResolved := core.ToMapAny(env["COCKTAIL_RECIPE_TEST_LOOKUP_ENTID"])
