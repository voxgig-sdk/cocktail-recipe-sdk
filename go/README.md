...
The Golang SDK for the CocktailRecipe API. Provides an entity-oriented interface using standard Go conventions — no generics required, data flows as `map[string]any`.


## Install
```bash
go get github.com/voxgig-sdk/cocktail-recipe-sdk
```
...
```bash
go mod edit -replace github.com/voxgig-sdk/cocktail-recipe-sdk=../path/to/github.com/voxgig-sdk/cocktail-recipe-sdk
```
...
```go
    result, err := client.Filter(nil).List(nil, nil)
    if err != nil {
        panic(err)
    }
...
```
github.com/voxgig-sdk/cocktail-recipe-sdk/
...
The root package (`github.com/voxgig-sdk/cocktail-recipe-sdk`) re-exports everything needed
