...
// CocktailRecipe SDK feature test
//
...
class FtCtrl extends CocktailRecipeControl
{
...
class FtClient
{
    ...
    private CocktailRecipeUtility $utility;

    public function __construct(CocktailRecipeUtility $utility, array $options)
    {
    ...
    public function get_utility(): CocktailRecipeUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CocktailRecipeContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CocktailRecipeContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CocktailRecipeContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CocktailRecipeConfig::make_feature($name);
    ...
    private static function buildUrl(CocktailRecipeSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CocktailRecipeError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CocktailRecipeResponse($fetched);
        }
    ...
    private function fail(CocktailRecipeContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CocktailRecipeContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
