...
// CocktailRecipe SDK retry feature
...
class CocktailRecipeRetryFeature extends CocktailRecipeBaseFeature
{
    ...
    public function init(CocktailRecipeContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CocktailRecipeContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CocktailRecipeContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CocktailRecipeContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
