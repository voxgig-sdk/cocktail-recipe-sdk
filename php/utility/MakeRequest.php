...
class CocktailRecipeMakeRequest
{
    public static function call(CocktailRecipeContext $ctx): array
    {
        ...
        if ($fetch_err) {
            ...
            $response = new CocktailRecipeResponse(['err' => $ctx->make_error('request_no_response', 'response: undefined')]);
        } elseif (is_array($fetched)) {
            $response = new CocktailRecipeResponse($fetched);
        } else {
