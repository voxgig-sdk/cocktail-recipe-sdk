...
// CocktailRecipe SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class CocktailRecipeMakePoint
{
    public static function call(CocktailRecipeContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof CocktailRecipeError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $point = $p;
                $select_def = CocktailRecipeHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = CocktailRecipeHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
